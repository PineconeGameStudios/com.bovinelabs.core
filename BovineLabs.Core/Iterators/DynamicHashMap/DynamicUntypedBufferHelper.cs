...
namespace BovineLabs.Core.Iterators
{
    ...
    public unsafe struct DynamicUntypedBufferHelper
    {
        ...
        internal int* Alignments
        {
            get
            {
                fixed (DynamicUntypedBufferHelper* data = &this)
                {
                    return (int*)((byte*)data + data->AlignmentsOffset);
                }
            }
        }
        ...
        internal static void Init(DynamicBuffer<byte> buffer, int capacity, int dataCapacity, int minGrowth)
        {
            ...
            var totalSize = CalculateDataSize(capacity, dataCapacity, out var offsetsOffset, out var sizesOffset, out var typesOffset, out var alignmentsOffset,
                out var dataOffset);
        ...
        internal static void Resize(DynamicBuffer<byte> buffer, ref DynamicUntypedBufferHelper* data, int newCapacity)
        {
            ...
            var totalSize = CalculateDataSize(newCapacity, data->DataCapacity, out var offsetsOffset, out var sizesOffset, out var typesOffset, out var alignmentsOffset,
                out var dataOffset);
            ...
            var oldAlignments = (int*)UnsafeUtility.Malloc(oldCapacity * sizeof(int), UnsafeUtility.AlignOf<int>(), Allocator.Temp);
            ...
            if (oldCapacity > 0)
            {
                ...
                UnsafeUtility.MemCpy(oldAlignments, data->Alignments, oldCapacity * sizeof(int));
            }
            ...
            if (oldCapacity > 0)
            {
                ...
                UnsafeUtility.MemCpy(data->Alignments, oldAlignments, oldCapacity * sizeof(int));
            }
            ...
            where TValue : unmanaged
        {
            ...
            var alignment = UnsafeUtility.AlignOf<TValue>();
            var dataAllocatedIndex = CollectionHelper.Align(data->DataAllocatedIndex, alignment);
        ...
        internal static void RemoveAt(DynamicBuffer<byte> buffer, ref DynamicUntypedBufferHelper* data, int index)
        {
            data->CheckIndexInRange(index);
            ...
            if (moveCount > 0)
            {
                ...
                UnsafeUtility.MemMove(alignments + index, alignments + index + 1, moveCount * sizeof(int));
            }
            ...
            for (var i = 0; i < data->Count; ++i)
            {
                ...
                dataIndex = CollectionHelper.Align(dataIndex, alignment);
            ...
            int capacity, int dataCapacity, out int offsetsOffset, out int sizesOffset, out int typesOffset, out int alignmentsOffset, out int dataOffset)
        {
            ...
            var alignmentsSize = sizeOfInt * capacity;
